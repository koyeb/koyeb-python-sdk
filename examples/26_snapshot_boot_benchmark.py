...
def setup_redis_with_data(sandbox: Sandbox, target_size_mb: int) -> None:
    ...
    result = sandbox.exec(
        "apk add --no-cache redis python3 py3-redis 2>/dev/null || apt-get update && apt-get install -y redis-server python3-pip && pip3 install redis",
        timeout=600,
    )
    if not result.success:
    ...
    result = sandbox.exec(
        "redis-server --daemonize yes --bind 127.0.0.1 2>/dev/null || redis-server --daemonize yes 2>/dev/null",
        timeout=30,
    )
    if not result.success:
...
"""
        ...
        if not result.success:
            raise BenchmarkError(
                f"Failed to load Redis batch {batch_start}-{batch_end}: {result.stderr}"
            )
...
def create_full_snapshot(
    sandbox: Sandbox, target_size_mb: int, suffix: str
) -> Snapshot:
    """
...
def benchmark_boot_from_snapshot(
    ...
    for boot_num in range(1, num_boots + 1):
        ...
        try:
            ...
            boot_name = (
                f"bench-{snapshot_type.lower()}-{size_safe}-{boot_num}-{random_str(4)}"
            )
...
def print_summary(results: List[BenchmarkResult]) -> None:
    ...
    for result in results:
        ...
        print(
            f"  Actual size: {result.snapshot_actual_size / (1024*1024):.0f}MB"
            if result.snapshot_actual_size
            else "  Actual size: N/A"
        )
        ...
        if result.warm_boot_times:
            ...
            print(
                f"  Min/Max:          {result.min_boot_time:.1f}s / {result.max_boot_time:.1f}s"
            )
...
def run_filesystem_benchmarks(
    ...
    for size_mb in sizes_mb:
        ...
        try:
            ...
            print(
                f"  ✓ Snapshot size: {snapshot.size / (1024*1024):.0f}MB"
                if snapshot.size
                else "  ✓ Snapshot size: N/A"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark snapshot boot times")
    parser.add_argument(
        "--fs-sizes",
        default="10,100,1000",
