#!/usr/bin/env python3
"""Benchmark exec latency for various trivial tasks (async variant).
...
Also benchmarks output retrieval with payloads of increasing size.
"""
...
async def run_benchmark(sandbox, label, exec_fn, pause=0.0):
    """Run the benchmark suite using the given async exec function.

    Args:
        sandbox: The sandbox instance.
        label: A human-readable label for the exec mode (e.g. "regular", "streaming").
        exec_fn: An async callable(cmd) -> CommandResult.
...
async def run_output_benchmark(sandbox, label, exec_fn, pause=0.0):
...
async def main(pause: float = 0.0):
    ...
    try:
        ...
        sandbox = await AsyncSandbox.create(
            image="koyeb/sandbox",
            name=f"bench-exec-async-{suffix}",
            wait_ready=True,
            api_token=api_token,
        )
        ...
        regular_summary = await run_benchmark(
            sandbox,
            "Regular",
        ...
        regular_out = await run_output_benchmark(
            sandbox,
            "Regular",
            lambda cmd: sandbox.exec(cmd),
            pause=pause,
        ...
        streaming_out = await run_output_benchmark(
            sandbox,
            "Streaming",
            lambda cmd: sandbox.exec(cmd, on_stdout=noop, on_stderr=noop),
            pause=pause,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark exec latency for trivial tasks — async variant (regular vs streaming, output retrieval)"
    )
    parser.add_argument(
        "--pause",
        type=float,
    ...
    sys.exit(asyncio.run(main(pause=args.pause)))
