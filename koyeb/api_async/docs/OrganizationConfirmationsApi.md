# koyeb.api_async.OrganizationConfirmationsApi
...
async with koyeb.api_async.ApiClient(configuration) as api_client:
    ...
    api_instance = koyeb.api_async.OrganizationConfirmationsApi(api_client)
    id = 'id_example' # str | 

    try:
        # Confirm organization action
        api_response = await api_instance.confirm_organization_action(id)
