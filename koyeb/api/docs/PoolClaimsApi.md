...
with koyeb.api.ApiClient(configuration) as api_client:
    ...
    body = koyeb.api.PoolClaimRequest() # PoolClaimRequest | 

    try:
        # Claim a sandbox from a service pool
        api_response = api_instance.claim(body)
...
with koyeb.api.ApiClient(configuration) as api_client:
    ...
    try:
        # List claims of a service pool
        api_response = api_instance.list_claim(pool_id, status=status, limit=limit, offset=offset)
