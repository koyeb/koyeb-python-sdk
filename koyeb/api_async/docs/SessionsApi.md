...
from koyeb.api_async.models.login_request import LoginRequest
...
async with koyeb.api_async.ApiClient(configuration) as api_client:
    ...
    api_instance = koyeb.api_async.SessionsApi(api_client)
    body = koyeb.api_async.LoginRequest() # LoginRequest | 
    seon_fp = 'seon_fp_example' # str | Seon Fingerprint (optional)

    try:
        # Login user DEPRECATED: this has been replaced by WorkOS and will be dropped soon.
        api_response = await api_instance.login(body, seon_fp=seon_fp)
...
async with koyeb.api_async.ApiClient(configuration) as api_client:
    ...
    try:
        # New session
        api_response = await api_instance.new_session()
