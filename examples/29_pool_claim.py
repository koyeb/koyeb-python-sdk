...
from koyeb import PoolClaim, Sandbox
...
def main():
    ...
    try:
        ...
        sandbox = Sandbox.get_from_id(id=claim.service_id, api_token=api_token)
        result = sandbox.exec("echo 'Hello from a claimed sandbox!'")
        print(f"  Output: {result.stdout.strip()}")

        # Replay demo: the same (pool_id, request_id) returns the same claim
        replay = PoolClaim.claim(
            pool_id=pool_id,
