"""Koyeb latency benchmark: exec, health check, and HTTP round-trip.
...
Usage:
    KOYEB_API_TOKEN=<token> uv run python examples/24_latency.py
    KOYEB_API_TOKEN=<token> uv run python examples/24_latency.py --debug
"""
...
def bench_health(sandbox: Sandbox) -> None:
...
def bench_exec(sandbox: Sandbox, sandbox_id: str) -> None:
    print(f"--- get_from_id() + exec('true') x{N} ---")
    times: list[float] = []
    for i in range(N):
        t0 = time.perf_counter()
        sb = Sandbox.get_from_id(id=sandbox_id, api_token=TOKEN)
        t1 = time.perf_counter()
        sb.exec("true")
...
def bench_exec_cached(sandbox: Sandbox, sandbox_id: str) -> None:
...
def bench_http_post(sandbox: Sandbox) -> None:
    sandbox.filesystem.write_file("/tmp/server.py", SERVER_SCRIPT)
    print("  Starting HTTP server on :8080...")
    sandbox.launch_process("python3 /tmp/server.py")
    time.sleep(2)
...
def main(debug: bool = False) -> None:
    ...
    try:
        bench_health(sandbox)
        bench_exec(sandbox, sandbox_id)
        bench_exec_cached(sandbox, sandbox_id)
        bench_http_post(sandbox)
    finally:
        sandbox.delete()
        print("Done.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Koyeb latency benchmark")
