...
with koyeb.api.ApiClient(configuration) as api_client:
    ...
    try:
        # List all Snapshots
        api_response = api_instance.list_snapshots(limit=limit, offset=offset, organization_id=organization_id, statuses=statuses, region=region, ids=ids)
