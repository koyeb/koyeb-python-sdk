...
async with koyeb.api_async.ApiClient(configuration) as api_client:
    ...
    api_instance = koyeb.api_async.ComposeApi(api_client)
    compose = koyeb.api_async.CreateCompose() # CreateCompose | 

    try:
        # Create resources from compose.
        api_response = await api_instance.compose(compose)
