# koyeb.api_async.RegionalDeploymentsApi
...
async with koyeb.api_async.ApiClient(configuration) as api_client:
    ...
    api_instance = koyeb.api_async.RegionalDeploymentsApi(api_client)
    id = 'id_example' # str | The id of the regional deployment

    try:
        # Get Regional Deployment
        api_response = await api_instance.get_regional_deployment(id)
...
async with koyeb.api_async.ApiClient(configuration) as api_client:
    ...
    try:
        # List Regional Deployment events
        api_response = await api_instance.list_regional_deployment_events(regional_deployment_id=regional_deployment_id, types=types, limit=limit, offset=offset, order=order)
