...
class TestPoolClaim(unittest.TestCase):
    ...
    def test_claim_error_detail_includes_response_body(self):
        # A non-retryable failure surfaces reason plus the decoded response
        # body in the error detail.
        claim_mock = MagicMock(
            side_effect=ApiException(
                status=400,
                reason="bad request",
                body=b'{"message": "pool drained"}',
            )
        )
        with patch(
            "koyeb.sandbox.pool.get_api_clients", return_value=_sync_clients(claim_mock)
        ...
        self.assertIn("pool drained", str(ctx.exception))
...
class TestPoolClaimGetClaim(unittest.TestCase):
    ...
    def test_get_claim_fetches_claim_by_id(self):
        ...
        with patch("koyeb.sandbox.pool.get_api_clients", return_value=clients):
            resource = PoolClaim.get_claim("cl-1")
    ...
    def test_get_claim_raises_when_reply_has_no_claim(self):
        get_claim_mock = MagicMock(return_value=GetPoolClaimReply(claim=None))
        clients = MagicMock()
        clients.pool_claims.get_claim = get_claim_mock
        with patch("koyeb.sandbox.pool.get_api_clients", return_value=clients):
            with self.assertRaises(SandboxClaimError) as ctx:
                PoolClaim.get_claim("cl-1")
        self.assertIn("cl-1", str(ctx.exception))
...
class TestAsyncPoolClaim(unittest.TestCase):
    ...
    def test_claim_generates_request_id_once(self):
        ...
        with patch("koyeb.sandbox.pool.get_async_api_clients", return_value=clients):
            claim = asyncio.run(AsyncPoolClaim.claim(POOL_ID, wait_ready=False))
        self.assertTrue(claim.request_id)
        self.assertEqual(bodies[0].request_id, claim.request_id)
    ...
    def test_claim_rejects_invalid_poll_interval(self):
        for bad in (0, -1):
            with self.assertRaises(ValueError):
                asyncio.run(AsyncPoolClaim.claim(POOL_ID, poll_interval=bad))
    ...
    def test_claim_wait_ready_timeout(self):
        ...
        with patch("koyeb.sandbox.pool.get_async_api_clients", return_value=clients):
            with self.assertRaises(SandboxTimeoutError):
                asyncio.run(
                    AsyncPoolClaim.claim(
                        POOL_ID, wait_ready=True, timeout=1, poll_interval=0.01
                    )
                )
    ...
    def test_get_claim_fetches_claim_by_id(self):
        ...
        with patch("koyeb.sandbox.pool.get_async_api_clients", return_value=clients):
            resource = asyncio.run(AsyncPoolClaim.get_claim("cl-1"))
        self.assertEqual(resource.id, "cl-1")
        self.assertEqual(
    ...
    def test_get_claim_requires_claim_id(self):
        with self.assertRaises(ValueError):
            asyncio.run(AsyncPoolClaim.get_claim(""))

    def test_get_claim_raises_when_reply_has_no_claim(self):
        clients = MagicMock()
        clients.pool_claims.get_claim = AsyncMock(
            return_value=AsyncGetPoolClaimReply(claim=None)
        )
        with patch("koyeb.sandbox.pool.get_async_api_clients", return_value=clients):
            with self.assertRaises(SandboxClaimError) as ctx:
                asyncio.run(AsyncPoolClaim.get_claim("cl-1"))
        self.assertIn("cl-1", str(ctx.exception))

    def test_wait_ready_rejects_invalid_poll_interval(self):
        for bad in (0, -1):
            with self.assertRaises(ValueError):
                asyncio.run(self._async_wait_claim().wait_ready(poll_interval=bad))

    def test_wait_ready_keeps_polling_when_status_missing(self):
        replies = [
            AsyncGetServiceReply(service=AsyncService(id=SERVICE_ID, status=None)),
            _async_service_reply(AsyncServiceStatus.HEALTHY),
        ]
        calls = []
        ...
        with patch("koyeb.sandbox.pool.get_async_api_clients", return_value=clients):
            self.assertTrue(
                asyncio.run(
                    self._async_wait_claim().wait_ready(timeout=5, poll_interval=0.01)
                )
            )
    ...
    def test_wait_ready_treats_other_schema_errors_as_transient(self):
        ...
        clients.services.get_service = _async_side_effect(
            [
                _async_other_field_validation_error(),
                _async_service_reply(AsyncServiceStatus.HEALTHY),
            ]
        )
        with patch("koyeb.sandbox.pool.get_async_api_clients", return_value=clients):
...
def _async_other_field_validation_error() -> ValidationError:
    """A real pydantic failure on a non-status field of the async model."""
    try:
        AsyncGetServiceReply.model_validate(
            {
                "service": {
