...
from koyeb.api_async.models.get_metrics_reply import GetMetricsReply
...
async with koyeb.api_async.ApiClient(configuration) as api_client:
    ...
    api_instance = koyeb.api_async.MetricsApi(api_client)
    ...
    try:
        # Get Metrics
        api_response = await api_instance.get_metrics(service_id=service_id, instance_id=instance_id, name=name, start=start, end=end, step=step)
